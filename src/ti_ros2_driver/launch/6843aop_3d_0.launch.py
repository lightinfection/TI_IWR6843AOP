...
def generate_launch_description():
    return LaunchDescription([
        ...
        executable="pcl_pub",
        parameters=[
            {'cfg_path': str(get_package_share_path('ti_ros2_driver') / 'cfg/base.cfg')},                 # cfg file for ti mmwave
            ...
            {'output_RA_heatmap': False},                                                                 # if output Range-Azimuth heatmap
            {"debug_mode": False},                                                                        # if using debug mode
            {"debug_log_path": str(get_package_share_path('ti_ros2_driver') / 'debug/result.json')},      # if using debug mode, the path of output debug log
        ],
        output="screen",
        emulate_tty=True
        )
