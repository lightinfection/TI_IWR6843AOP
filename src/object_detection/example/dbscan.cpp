...
public:
  ...
  : Node("dbscan_cluster")
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("dbscan_cluster"), "Subscribed! \n");
    if(db.success) 
    {
    ...
    else RCLCPP_ERROR(rclcpp::get_logger("dbscan_cluster"), "parameters setting went wrong\n");
  }
...
private:
  ...
  void get_param()
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("dbscan_cluster"), "set done: %.2f, %d, %d, %s, %s \n", r, num, bb, frame.c_str(), node_name.c_str());
  }
